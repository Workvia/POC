...
const GENERATE_ATTACHMENT_SEEDS = (): AttachmentDataSeed[] => {
  ...
  for (let INDEX = 1; INDEX <= 400; INDEX++) {
    ...
    // Distribution: ~32% person, ~32% company, ~21% note, ~15% task
    ...
    if (DISTRIBUTION_VALUE < 32) {
      // 32% Person attachments
      personId = PERSON_IDS[entityIndex % PERSON_IDS.length];
      entityIndex++;
    } else if (DISTRIBUTION_VALUE < 64) {
      // 32% Company attachments
      companyId = COMPANY_IDS[entityIndex % COMPANY_IDS.length];
      entityIndex++;
    } else if (DISTRIBUTION_VALUE < 85) {
      // 21% Note attachments
